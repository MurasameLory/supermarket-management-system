...
public class SmallScrollPanel extends JPanel {
	...
	public SmallScrollPanel() {
		scrollMouseAdapter = new ScrollMouseAdapter(); //鼠标识别器
		initialize();
	}
	...
	private JButton getLeftScrollButton() {
		if(leftScrollButton==null) {
			...
			ImageIcon icon1= new ImageIcon(getClass().getResource("../icon/left_off.png"));
			ImageIcon icon2= new ImageIcon(getClass().getResource("../icon/left_on.png"));
	...
	private JButton getRightScrollButton() {
		if(rightScrollButton==null) {
			...
			ImageIcon icon1= new ImageIcon(getClass().getResource("../icon/right_off.png"));
			ImageIcon icon2= new ImageIcon(getClass().getResource("../icon/right_on.png"));
	...
	private class ScrollButtonShowListener extends ComponentAdapter implements Serializable {
		...
		public void componentResized(ComponentEvent e) {
			...
			if(paneWidth >= scrollWidth) {
				getLeftScrollButton().setVisible(false); //尺寸发生改变，滚动条是否存在。
	...
	private final class ScrollMouseAdapter extends MouseAdapter implements Serializable {
		...
		public void mousePressed(MouseEvent e) {
			...
			isPressed = true;			//是否按下
			if(source==getLeftScrollButton()) {  //向左滚动的按钮
				scrollMoved(-1);
			}else {
				scrollMoved(1);
			}
